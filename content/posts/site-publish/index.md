...
{{< lead >}}此文是上一篇《关于本站的建造》的延申，是关于把已搭建的 Hugo 站点进行部署和加速的文章。与上一篇不同，这是一篇普适性教程。{{</ lead >}}
<!-- {{< lead >}}这是由上一篇《关于本站的建造》引申出来的文章，主要讲解的是把构建好的 Hugo 网站进行部署和加速的过程。与上一篇不同，这是一篇普遍使用的教程。{{</ lead >}} -->
{{< alert >}}敬请留意：此文撰写于多年以前。时过境迁，本站现已挂载于 Cloudflare 上，已脱离国内的网络服务框架；本文所述内容可能仍有指引性意义，但本站或许已不采用下文的某些方式或服务。{{</ alert >}}
## 部署网站  
### 部署到GitHub Pages  
